...
function checkForVictory() {
  for (const row in CORRECT_ANSWERS) {
    const correctWords = CORRECT_ANSWERS[row];

    const guessedUniqueIds = guessesState[row] || [];

    const guessedWords = guessedUniqueIds.map(id => {
      const word = id.includes('_') ? id.split('_')[0] : id;
    ...
    if (guessedWords.length !== normalizedCorrect.length) {
      console.log(`Row ${row} length mismatch`, guessedWords, normalizedCorrect);
      return false;
    }

    for (let i = 0; i < normalizedCorrect.length; i++) {
      if (guessedWords[i] !== normalizedCorrect[i]) {
        console.log(`Row ${row} word mismatch at position ${i}`);
        console.log("Guessed:", guessedWords[i]);
        console.log("Correct:", normalizedCorrect[i]);
        return false;
      }
    }
  }
...
function setupDropTargets() {
  ...
  document.querySelectorAll(".drop-zone").forEach(zone => {
    ...
    zone.addEventListener("drop", e => {
      ...
      if (checkForVictory()) {
        console.log("Calling victory screen...");
  showVictoryScreen();
}
    });
